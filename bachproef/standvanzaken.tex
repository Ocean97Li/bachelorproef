...
\label{sec:blockchain}
	...
	In dit geval wordt door de auteur er gekozen voor de gedistribueerde techniek. In de volgende paragraaf wordt een methode besproken om niet langer op basis van vertrouwen te werken.
...
\label{sec:blockchain-gebaseerd-stemmen}
	...
	\subsection*{Cryptografie en Stemprotocollen}
		...
		Kiayias and Yung [24] en Khader et al. stellen dat dit makkelijk te corrigeren valt doormiddel van een extra stemronde. Mcorry stelt echter dat daarvoor de volledige coöperatie van alle participanten nodig is, en dat deze niet meer gegarandeerd is op dit punt. Ook wordt er voorgesteld om de laatste stem steeds een lege stem van de organisator te laten zijn, maar ook hiertegen verzet Mcorry zich, men stelt dat dit in essentie een terugkeer is naar een systeem met centrale autoriteit. 
		Mcorry et al. stelt een protocol voor, op basis van het werk van Kiayias and Yung en anderen maar combineert als eerste het concept van self-tallying met een blockchain implementatie. Het resulteren protocol wordt het Open Vote Network Protocol.
